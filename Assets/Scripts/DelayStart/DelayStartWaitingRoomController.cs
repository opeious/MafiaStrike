...
public class DelayStartWaitingRoomController : MonoBehaviourPunCallbacks
{
    ...
    [SerializeField] private GameObject searchingContainer;
    [SerializeField] private GameObject foundContainer;
    ...
    void PlayerCountUpdate()
    {
        ...
        if (playerCount > 1)
        {
            ...
            foundContainer.SetActive(true);
            btnText.text = "STARTING";
        }
        else
        {
            searchingContainer.SetActive(true);
            foundContainer.SetActive(false);
            btnText.text = "CANCEL SEARCH";
        }
    ...
    private void Update()
    {
        textAnim += Time.deltaTime;
        if (textAnim > 1f)
        {
            textAnim = 0f;
            animIndex++;
            if (animIndex > textsForAnim.Count - 1)
            {
                animIndex = 0;
            }

            animText.text = textsForAnim[animIndex];
//            animText.text
        } 
    ...
    private List<string> textsForAnim = new List<string>();
    ...
    void StartGame()
    {
        ...
        PhotonNetwork.LoadLevel("BetterMPScene");
    }
